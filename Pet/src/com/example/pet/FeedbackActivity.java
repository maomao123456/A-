	...
	private void addFeedback(){
		new Thread(new Runnable(){
			public void run(){
				...
				try{
					String httpPost = "http://192.168.1.145/index.php/Home/Pet/addFeedback";
				...
				params.add(new BasicNameValuePair("feedback", inputFeedback.getText().toString().trim()));//添加用户反馈的意见或遇到的问题
				params.add(new BasicNameValuePair("conphone", contantPhone.getText().toString().trim()));//天机用户的联系电话
				try {
					HttpEntity httpEntity = new UrlEncodedFormEntity(params, "utf-8");//设置用户字符集的格式
					...
					if(httpResponse.getStatusLine().getStatusCode() == 200){//http客户端响应成功
						...
						Looper.prepare();
						Toast.makeText(getApplicationContext(), strResult + "提交成功", Toast.LENGTH_SHORT).show();
						Looper.loop();
					} else {
						Looper.prepare();
